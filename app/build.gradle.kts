...
configure<ApplicationExtension> {
    ...
    signingConfigs {
        create("notubeRelease") {
            storeFile = rootProject.file("notube-release.jks")
            storePassword = "notube123"
            keyAlias = "notube"
            keyPassword = "notube123"
        }
    }

    defaultConfig {
        applicationId = "com.notube.app"
        resValue("string", "app_name", "NoTube")
        minSdk = 26
        targetSdk = 35
    ...
    buildTypes {
        debug {
            ...
            if (normalizedWorkingBranch.isEmpty() || workingBranch in defaultBranches) {
                ...
                resValue("string", "app_name", "NoTube Debug")
            } else {
                applicationIdSuffix = ".debug.$normalizedWorkingBranch"
                resValue("string", "app_name", "NoTube $workingBranch")
            }
        }
        ...
        release {
            System.getProperty("packageSuffix")?.let { suffix ->
                ...
                resValue("string", "app_name", "NoTube $suffix")
            }
            isMinifyEnabled = true
            isShrinkResources = true
            signingConfig = signingConfigs.getByName("notubeRelease")
            proguardFiles(
